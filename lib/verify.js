  ...
  if (pluginConfig?.publish !== false) {
    ...
    if (!vscePublishEnabled && !ovsxPublishEnabled) {
      throw new SemanticReleaseError(
        'No personal access token was detected. Set the `VSCE_PAT`, `VSCE_AZURE_CREDENTIAL`, or the `OVSX_PAT` environment variable, at least one of them must be present when publish is enabled.\nLearn more at https://github.com/felipecrs/semantic-release-vsce#publishing',
        'ENOPAT',
      );
    }
